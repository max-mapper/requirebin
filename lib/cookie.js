...
cookie.set = function (name, value, expires, path, domain) {
  ...
  var pathTruthy = !!path
  pair += ';path=' + ((pathTruthy) ? path : '/')

  var domainTruthy = !!domain
  if (domainTruthy) pair += ';domain=' + domain

  document.cookie = pair
  return cookie
}

cookie.setObject = function (object, expires, path, domain) {
